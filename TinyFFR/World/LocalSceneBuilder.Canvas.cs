...
sealed partial class LocalSceneBuilder {
	...
	public void PrepareCanvasObjectsForRender(ResourceHandle<Scene> handle, XYPair<int> viewportSize, XYPair<int> subAreaBottomLeft, XYPair<int> targetSize) {
		...
		if (objectsNeedPlacementAdjustment) {
			foreach (var item in _canvasItemMap[handle].Values) SetCanvasItemTransformAccordingToViewport(in item, viewportSize);	
		}
	}
	
	void SetCanvasItemTransformAccordingToViewport(in CanvasItemData itemData, XYPair<int> viewportSize) {
		...
		if (itemData.Quad is { } quad) {
			quad.SetTransform(position, size, CanvasElementFacingDirection, CanvasElementPositiveYDirection, dock.EffectiveElementAnchor);
		}
		else {
			...
			text.SetTransform(
				...
				new TextLayout(size.Y > 0f ? size.Y : null, dock.EffectiveElementAnchor, size.X > 0f ? size.X : null, false)
			);
		}
	}
	...
	// 	SetCanvasItemTransformAccordingToViewport(handle, in data);
	// }
	//
	...
	// 	SetCanvasItemTransformAccordingToViewport(handle, in newData);
	// }
	//
