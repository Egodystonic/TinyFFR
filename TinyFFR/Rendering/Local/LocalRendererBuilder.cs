...
sealed class LocalRendererBuilder : IRendererBuilder, IRendererImplProvider, IResourceDirectory<Renderer>, IResourceDirectory<RenderOutputBuffer>, IResourceDirectory<RendererCompositor>, IDisposable {
	...
	readonly struct RenderTargetUnion : IRenderTarget, IEquatable<RenderTargetUnion> {
		...
		public override string ToString() => IsWindow ? AsWindow.ToString() : AsBuffer.ToString();
	}
	...
	readonly struct DesiredViewportDimensionsUnion {
		...
		public (XYPair<int> BottomLeft, XYPair<int> Size) ExtractViewportPixelBounds(XYPair<int> renderTargetSize) {
			...
			if (SpecifiedAsPixel) {
				...
				offsetPx = (rtSizeFloat * OffsetFraction).CastWithRoundingIfNecessary<float, int>();
	...
	public void RenderAll(ResourceHandle<RendererCompositor> handle) {
		...
		if (compositorData.NumRenderersCurrentlyEnabled == 1) {
			...
			RenderInternal(sole.Renderer.Handle, RenderOrdering.Standalone, sole.CompositionType);
			return;
		}
		...
		RenderInternal(first.Renderer.Handle, RenderOrdering.First, first.CompositionType);
		try {
			for (var i = compositorData.FirstEnabledRendererIndex + 1; i < compositorData.LastEnabledRendererIndex; ++i) {
				...
				RenderInternal(middle.Renderer.Handle, RenderOrdering.Middle, middle.CompositionType);
			}
		}
		finally {
			RenderInternal(last.Renderer.Handle, RenderOrdering.Last, last.CompositionType);
		}
	}
	...
	unsafe void RenderInternal(ResourceHandle<Renderer> handle, RenderOrdering ordering, RenderCompositionType compositionType) {
		...
		if (rendererData.LastPushedCompositingMode != desiredCompositing) {
			SetViewCompositingMode(rendererData.Viewport.Handle, desiredCompositing.Translucent, desiredCompositing.ClearDepth).ThrowIfFailure();
			rendererData = rendererData with { LastPushedCompositingMode = desiredCompositing };
			_loadedRenderers[handle] = rendererData;
		}
		...
		if (isWindow) {
			RenderScene(
				...
				ordering is RenderOrdering.Standalone or RenderOrdering.First,
				ordering is RenderOrdering.Standalone or RenderOrdering.Last
			).ThrowIfFailure();
		}
			...
			if (!performReadback || !bufferData.RenderCompletionHandlers.AnySet) {
				...
				if (bufferData.HandleOnlyNextChange) {
					_loadedBuffers[bufferData.Handle] = _loadedBuffers[bufferData.Handle] with {
						RenderCompletionHandlers = OutputBufferCallbackData.None,
		...
		if (rendererData.EmitFences && ordering is RenderOrdering.Standalone or RenderOrdering.Last) {
	...
	[DllImport(LocalNativeUtils.NativeLibName, EntryPoint = "set_view_compositing_mode")]
	static extern InteropResult SetViewCompositingMode(
		UIntPtr viewDescriptorHandle,
	...
	static extern InteropResult RenderScene(
		...
		UIntPtr viewDescriptorHandle,
		UIntPtr swapChainHandle,
		InteropBool invokeBeginFrame,
		InteropBool invokeEndFrame
	);
	...
	#region Disposal
	bool RenderTargetUnionIsInUse(RenderTargetUnion rtu) {
		foreach (var rendererData in _loadedRenderers.Values) {
			if (rendererData.RenderTarget == rtu) return true;
		}
		foreach (var compositorData in _loadedCompositors.Values) {
			if (compositorData.RenderTarget == rtu) return true;
		}
	...
	public void Dispose(ResourceHandle<RendererCompositor> handle, bool disposeContainedRenderers) {
		...
		for (var i = data.AddedRenderers.Count - 1; i >= 0; --i) {
			_globals.DependencyTracker.DeregisterDependency(HandleToInstance(handle), data.AddedRenderers[i].Renderer);
			if (disposeContainedRenderers) Dispose(data.AddedRenderers[i].Renderer.GetHandleWithoutDisposeCheck());
		}
	...
	public void Dispose() {
		...
		try {
			...
			while (_loadedCompositors.Count > 0) {
				Dispose(_loadedCompositors.GetPairAtIndex(0).Key);
			}
			...
			_compositedRendererVectorPool.Dispose();
		}
		finally {
			_isDisposed = true;
		}
