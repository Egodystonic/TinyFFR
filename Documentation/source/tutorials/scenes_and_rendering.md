...
:   This sets how close something has to be to the camera before it is no longer rendered.
	...
	The default value is `CameraCreationConfig.DefaultNearPlaneDistance` (0.03m). This can not be 0 due to [perspective divide](https://stackoverflow.com/questions/17269686/why-do-we-need-perspective-division); the lowest permitted value is `Camera.NearPlaneDistanceMin` (1E-5m).
...
:   This sets how far something can be from the camera before it is no longer rendered.
	...
	The default value is `CameraCreationConfig.DefaultFarPlaneDistance` (1000m). This can not be lower than or equal to `NearPlaneDistance`.
