...
TinyFFR's [input abstraction](input.md) is available when using WPF integration, but it is sourced from WPF's own input events rather than from TinyFFR's usual (SDL-based) event polling.

* All interaction with your UI and TinyFFR should be done in the UI Dispatcher context (e.g. on the UI thread).
* To use TinyFFR's input abstraction, use the `StartWpfUiLoop()` overload that supplies an `ILatestInputRetriever` (see "Input" below). You may of course continue to use WPF's built-in input layer instead (or as well).

???+ warning "Do not enable TinyFFR's own input polling"
	When using TinyFFR standalone, the `ApplicationLoop` polls the operating system for input events on each iteration. Doing that whilst WPF owns the message loop would consume events that the UI framework needs, causing your application to "miss" input.

	For this reason you should leave the config value `IterationShouldRefreshGlobalInputStates` set to `false` on any `ApplicationLoop` you create in a WPF application (the `StartWpfUiLoop()` functions described below do this for you). Input is instead supplied by the UI framework, which is why the retriever must be told which element to observe.
...
An overload of `StartWpfUiLoop()` accepts an `Action<TimeSpan, ILatestInputRetriever>` together with the element whose input should be tracked. This lets you reuse the same input-handling code you would write for a standalone TinyFFR application:
...
var loopTerminationDisposable = factory.ApplicationLoopBuilder.StartWpfUiLoop(Tick, sceneView);
```

<span class="def-icon">:material-code-block-parentheses:</span> `StartWpfUiLoop(tickCallback, inputSource, tickRateHz, priority, name)`
